...
exports.updateResponse = async (req, res) => {
    try {
      const { id } = req.params; // This will contain the orderNumber
      const { oldPowercord, wireAvailable, howMuchWireAvailable, storeRemarks } = req.body;
  
      const response = await StoreResponse.findOne({ where: { orderNumber: id } });
  
      if (!response) {
        console.error(`Response not found for Order Number: ${id}`);
        return res.status(404).json({ error: `Response not found for Order Number: ${id}` });
      }
      ...
      res.status(500).json({ error: 'Failed to update store response', details: error.message });
    }
  };
